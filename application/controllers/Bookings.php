...
class Bookings extends CI_Controller {
    ...
    public function bookingAdd($id = null, $confirm = false) {
		...
		if($confirm && !$data['user']['disabled'] && $data['user']['role'] == 1) {
			$this->BookingModel->bookingCancel($id, $to_update);
			$this->session->set_flashdata('alert', array('type' => 'alert alert-success', 'msg'  => 'Successfully add booking.'));
        }

		return redirect(BOOKINGS_CONTROLLER);
	}
    public function bookingInvoice($id = null, $confirm = false) {
		...
		if($confirm && !$data['user']['disabled'] && $data['user']['role'] == 1) {
			$this->BookingModel->bookingCancel($id, $to_update);
			$this->session->set_flashdata('alert', array('type' => 'alert alert-success', 'msg'  => 'Successfully print invoice.'));
        }
